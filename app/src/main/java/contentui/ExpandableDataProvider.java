...
public class ExpandableDataProvider extends AbstractExpandableDataProvider {
    ...
    public ExpandableDataProvider() {
        ...
        chapterData = getPartsAndChapters(4);
        for (int i = 0; i < chapterData.size(); i++) {
            Log.d("Frank ", "onCreate: part = " + chapterData.get(i).getPart() + ", chapter = " +
        ...
        for (int i = 0; i < chapterData.size(); i++) {
            int bookPart = chapterData.get(i).getPart();
            int chapterNumber = chapterData.get(i).getChapter();
            long chapterDuration = chapterData.get(i).getDuration();
            if (bookPart != part) {
                part = bookPart;
                group = new ConcreteGroupData(bookPart);
                children = new ArrayList<>();
            }
            final long childId = group.generateNewChildId();
            children.add(new ConcreteChildData(childId, chapterNumber, chapterDuration));
        }
    }
...
public static final class ConcreteChildData extends AbstractExpandableDataProvider.ChildData {
    ...
    private final long mChapterduration;
    private boolean mPinned;

    ConcreteChildData(long id, int chapNumber, long chapDuration) {
        mId = id;
        mChapterNumber = chapNumber;
        mChapterduration = chapDuration;
    }
    ...
    @Override
    public String getText() {
        return ((Integer) mChapterNumber).toString();
    }

    public long getDurration() {
        return mChapterduration;
    }
